...
async function run() {
  try {
    ...
    app.post('/properties', verifyToken, verifyOwner, async (req, res, next) => {
      try {
    ...
    app.get('/properties/owner/:email', verifyToken, verifyOwner, async (req, res, next) => {
      try {
        const email = req.params.email
    ...
    app.delete('/properties/:id', verifyToken, async (req, res, next) => {
      try {
    ...
    app.patch('/properties/status/:id', verifyToken, verifyAdmin, async (req, res, next) => {
      try {
        const id = req.params.id
    ...
    app.get('/properties/all', verifyToken, verifyAdmin, async (req, res, next) => {
      try {
    ...
    app.post('/bookings/confirm-payment', verifyToken, async (req, res, next) => {
      try {
    ...
    app.get('/bookings/tenant/:email', verifyToken, async (req, res, next) => {
      try {
        const email = req.params.email
    ...
    app.post('/favorites', verifyToken, async (req, res, next) => {
      try {
    ...
    app.get('/favorites/my-favorites', verifyToken, async (req, res, next) => {
      try {
        const email = req.decoded.email
    ...
    app.delete('/favorites/:id', verifyToken, async (req, res, next) => {
      try {
        const id = req.params.id
    ...
    app.get('/reviews/property/:id', async (req, res, next) => {
      try {
        const id = req.params.id
    ...
    app.get('/owner/analytics', verifyToken, verifyOwner, async (req, res, next) => {
      try {
        const email = req.decoded.email
    ...
    app.get('/properties/my-properties', verifyToken, verifyOwner, async (req, res, next) => {
      try {
        const email = req.decoded.email
        const result = await propertiesCollection.find({ 'owner.email': email }).toArray()
        res.send(result)
    ...
    app.patch('/properties/:id', verifyToken, verifyOwner, async (req, res, next) => {
      try {
        const id = req.params.id
    ...
    app.get('/bookings/owner/:email', verifyToken, verifyOwner, async (req, res, next) => {
      try {
        const email = req.params.email
        if (req.decoded.email !== email) {
          return res.status(403).send({ message: 'Forbidden access' })
    ...
    app.patch('/bookings/status/:id', verifyToken, verifyOwner, async (req, res, next) => {
      try {
        const id = req.params.id
    ...
    app.patch('/users/role/:email', verifyToken, verifyAdmin, async (req, res, next) => {
      try {
        const email = req.params.email
        const { role } = req.body
    ...
    app.patch('/users/status/:id', verifyToken, verifyAdmin, async (req, res, next) => {
      try {
        const id = req.params.id
        const { status } = req.body
        const result = await usersCollection.updateOne(
    ...
    app.get('/admin/transactions', verifyToken, verifyAdmin, async (req, res, next) => {
      try {
    ...
    app.get('/bookings/report/:id', verifyToken, async (req, res, next) => {
      try {
        const id = req.params.id
